...
const MessageCountStats: React.FC<{
    ...
    interval?: number,
    maxLength?: number,
}> = ({serverId,interval = 1000,maxLength = 40}) => {
    ...
    return <RTMultiLineChartCard
        ...
        fetchValues={() => {
            ...
            if(!server) return [0,0,0,0];
            return [
                server.wsMessageCount,
                server.transmitMessageCount,
                server.invokeMessageCount,
                server.httpMessageCount
            ]
        }}
